...
public class VisUdlejningWindow extends Stage{
	...
	private void initContent(GridPane pane) {
		...
		pane.add(lblPris, 0, 9);
		
		double totalPris = 0.0;
		ArrayList<Double> priser = udlejning.beregnPris();
		for(double pris : priser) {
			totalPris += pris * (udlejning.getTilbageleveringsdato().getDayOfYear() - udlejning.getUdleveringsdato().getDayOfYear());
		}
		txfPris = new TextField();
		pane.add(txfPris, 0, 10);
		txfPris.setText("" + totalPris);
		txfPris.setEditable(false);
		
		Button btnClose = new Button("Close");
		pane.add(btnClose, 0, 11);
