...
public class UdlejningWindow extends Stage {
	...
	private void removeBilAction() {
		if(!lvwBiler.getItems().isEmpty()) {
			lvwBiler.getSelectionModel().getSelectedItem().setTilstand(Tilstand.KLAR);
	...
	private void createUdlejningAction() {
		...
		if(!lvwBiler.getItems().isEmpty() && ldUd.isBefore(ldTil)) {
			...
			ArrayList<Bil> biler = new ArrayList<>(lvwBiler.getItems());
			
			Udlejning udlejning = Service.createUdlejning(kunde, biler, ldUd, ldTil);
			kunde.addUdlejninger(udlejning);
			System.out.println(Storage.getUdlejninger());
			this.hide();
