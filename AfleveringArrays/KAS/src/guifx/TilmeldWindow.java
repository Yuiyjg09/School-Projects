...
public class TilmeldWindow extends Stage {  
    ...
    private void selectedHotelChanged() {
    	if(hotel.getSelectionModel().getSelectedIndex() >= 0) {
	...
	private void tilmeldAction() {
        ...
        if (dNavn.length() == 0 || dTlf.length() == 0 || dEmail.length() == 0 || dAdr.length() == 0 || dFirma.length() == 0 || dFirmaTlf.length() == 0 || dLandby.length() == 0 || ankomst.getValue().isBefore(konference.getDatoTid())) {
